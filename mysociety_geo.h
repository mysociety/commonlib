...
//
// $Id: mysociety_geo.h,v 1.1 2009-09-24 22:00:29 francis Exp $
//
...
void lat_lon_to_merc(double lat, double lon, double* x, double* y) {
    ...
    *x = 6378137 * *x * M_PI / 180;
    *y = 6378137 * *y;
}
void merc_to_lat_lon(double x, double y, double* lat, double* lon) {
    *lat = 180/M_PI * (2 * atan(exp((y/(6378137*M_PI/180))*M_PI/180)) - M_PI/2);
    *lon = x / 6378137 / M_PI * 180;
}
